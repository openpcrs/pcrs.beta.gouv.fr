...
test.serial('create valid projet', async t => {
  ...
  const testDate = new Date(projet.subventions[0].echeance)
  ...
  t.true(testDate instanceof Date && !Number.isNaN(testDate.valueOf()))
})

test.serial('create unvalid projet', async t => {
  const projet = {
    nom: 'pcrs invalide',
    regime: 'production',
    livrables: 'coucou'
